...
// [ComposeUpRequest] objects.  metadata.name is constructed by joining
// `status.namespace` and `status.name` with a dot if the result is a valid
// Kubernetes name; otherwise it is `cmp-` followed by the lower-case SHA256
// hash of that joined string.
