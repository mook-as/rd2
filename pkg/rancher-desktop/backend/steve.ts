  ...
  private async startInternal(): Promise<number> {
    ...
    stdout.on('end', () => {
      ...
      if (port) {
        ...
        portLatch.reject(new Error(`Failed to parse Steve port from pid ${ childProcess.pid }: ${ portBuffer }`));
      }
    });
    ...
    stderr.on('data', (data) => {
      console.error(`stderr(${ childProcess.pid }): ${ data }`);
    });

    childProcess.on('exit', (code, signal) => {
      if (childProcess !== this.process) {
        // A stale process has exited; ignore.
        console.debug(`Stale steve process ${ childProcess.pid } exited with code ${ code } and signal ${ signal }`);
        return;
      }
      console.log(`Steve process ${ childProcess.pid } exited with code ${ code } and signal ${ signal }`);
      this.#port = 0;
      send('backend/steve-port', 0);
    ...
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        const error = new Error(`Timed out waiting for Steve (${ childProcess.pid }) to start (${ portBuffer })`);
      ...
      childProcess.once('error', (err) => {
        clearTimeout(timeout);
        reject(new Error(`Failed to spawn Steve (${ childProcess.pid }): ${ err.message }`, { cause: err }));
      });
    });
